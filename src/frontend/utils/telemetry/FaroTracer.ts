...
const { NEXT_GRAFANA_FARO_APPNAME = '', NEXT_GRAFANA_FARO_ENDPOINT = '' } =
  typeof window !== 'undefined' ? window.ENV : {};

const Faro = async (collectorString: string) => {
  const url = NEXT_GRAFANA_FARO_ENDPOINT || collectorString;
  const appname = NEXT_GRAFANA_FARO_APPNAME || collectorString;
  ...
  if (url) {
    initializeFaro({
      ...
      app: {
        name: NEXT_GRAFANA_FARO_APPNAME || collectorString,
